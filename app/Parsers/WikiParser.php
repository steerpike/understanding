<?php
	...
	class WikiParser {
		...
		private function meta_boxes() {
			...
			foreach($matches[0] as $key => $raw_template_syntax) {
				...
				if(in_array($_meta_box_type, array("coord", "coords"))) {
					...
					$infobox_tmp = explode("|", $infobox_tmp);
					//real old:preg_match_all("#\s*(^|\|)\s*([^=]+)\s*=#si", $infobox_tmp, -1, PREG_SPLIT_DELIM_CAPTURE);
					
					if("" === $infobox_tmp[0]) {
						array_shift($infobox_tmp);
					}
					
					if(in_array($_meta_box_type, array("automatic_taxobox", "geobox")) && (false === strpos($infobox_tmp[0], "="))) {
					...
					foreach($infobox_tmp as $it_key => $line) {
						...
						if(true) {
							//if(!preg_match("#^([^=]+?)=(.*)$#si", $line, $line_match))
							if(!preg_match("#^([A-Za-z0-9\s\-\_]+?)=(.*)$#si", $line, $line_match)) {
								if(!isset($infobox_values[':etc'])) {
									$infobox_values[':etc'] = array();
								}
								
								$infobox_values[':etc'][] = trim($line);
								
								continue;
							}
							
							$row_key = $this->compact_to_slug($line_match[1]);
							$row_value = trim($line_match[2]);
						} else {
							if((++$i % 2) == 1) {
						...
						if((strlen($row_key) < 1) || (strlen($row_value) < 1) || ("?" === $row_value)) {
							continue;
						}
				...
				if(in_array($_meta_box_type, array("uscensuspop"))) {
					unset($infobox_values['align'], $infobox_values['footnote']);
				}
		...
		private function external_links() {
			...
			foreach($lines as $line) {
				...
				switch(true) {
					...
					case preg_match("#\{\{\s*(myspace|facebook|twitter)(?:\s*user)?\s*\|([^\}\}]+?)\}\}#si", $line_value, $lmatch):
						$bleh = explode("|", $lmatch[2]);
		...
		private function initial_clean() {
			...
			// Parse basic {{#expr: ... }} using math
			// http://en.wikipedia.org/wiki/Help:Expr
			$this->text = preg_replace_callback("#\{{2}\s*\#\s*expr\s*\:\s*([0-9\.\-\/\+\*x\s\(\)]+)\s*\}{2}#si", function($match) {
			...
			// Parse {{#ifeq}}
			// http://en.wikipedia.org/wiki/Help:Expr#Comparisons
			...
			$this->text = preg_replace_callback("#\{{2}\s*convert\s*\|([^\|]+)\|([^\||\}{2}]+)(?:\|.*?)?\}{2}#si", function ($match) {
		...
		private function insert_jungledb_helpers($text) {
			...
			// links
			$text = preg_replace_callback("#https?\:\/\/(?:[^\s]+)\|#si", function($match) {
				return str_replace("|", "###JNGLDBDEL###", $match[0]);
			}, $text);
			
			// inner-wiki links
			$text = preg_replace_callback("#\[{2}(?:.+?)\]{2}#si", function($matches) {
				return str_replace("|", "###JNGLDBDEL###", $matches[0]);
			}, $text);
		...
		private function explodeString_byBreak($string) {
			...
			$string = preg_replace("#(wiki\]|\]{2})\s*(,|;|\*|•)\s*(\[{2}|\[wiki)#si", "\\1<br />\\3", $string);
			
			$split = preg_split("#\s*<\s*?/?\s*?br\s*?/?\s*?>\s*#si", $string, -1, PREG_SPLIT_NO_EMPTY);
		...
		private function finalize_internal_links($value) {
			...
			$value = $this->clean_jungledb_helpers($value);
